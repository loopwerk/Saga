...
A sitemap tells search engines which pages exist on your site. Saga includes a built-in ``sitemap(baseURL:filter:)`` renderer that automatically includes all pages generated by your writers and ``Saga/createPage(_:using:)`` calls.
