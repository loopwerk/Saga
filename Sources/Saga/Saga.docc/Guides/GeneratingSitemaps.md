...
A sitemap tells search engines which pages exist on your site. Saga includes a built-in ``Saga/sitemap(baseURL:)`` renderer that automatically includes all pages generated by your writers and ``StepBuilder/createPage(_:using:)`` calls.
...
> Important: Order matters! ``Saga/sitemap(baseURL:)`` should be the last step in the pipeline, so that it sees all the previously generated pages.
