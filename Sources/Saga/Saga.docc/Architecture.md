...
2. Then, for every registered step, it passes those files to a matching ``Reader``. These readers turn text files (such as markdown files) into `Item` instances.
3. All unhandled files (images, CSS, raw HTML, etc.) are copied as-is to the `output` folder.
