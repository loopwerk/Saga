...
internal class AnyProcessStep {
  ...
  init<M: Metadata>(step: ProcessStep<M>, fileStorage: [FileContainer], inputPath: Path, outputPath: Path, itemWriteMode: ItemWriteMode, fileIO: FileIO) {
    runReaders = {
      ...
      let items = try await withThrowingTaskGroup(of: (FileContainer, Result<Item<M>?, Error>).self) { group in
          ...
          group.addTask(priority: .high) {
            do {
              ...
              if step.filter(item) {
                ...
                return (unhandledFileContainer, .success(item))
              } else {
                return (unhandledFileContainer, .success(nil))
              }
            } catch {
              // Couldn't convert the file into an Item, probably because of missing metadata.
              return (unhandledFileContainer, .failure(error))
            }
          }
        }
        
        var items = [Item<M>]()
        for try await (fileContainer, result) in group {
          switch result {
            ...
            case .failure(let error):
              print("❕File \(fileContainer.relativePath) failed conversion to Item<\(M.self)>, error: ", error)
          }
        }
